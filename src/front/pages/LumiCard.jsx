...
const buildDynamicMessages = (s) => {
  const msgs = [];
  if (s.urgentTasks > 0) {
    msgs.push({ tag: '🔥 Agenda', text: `Tienes ${s.urgentTasks} ${s.urgentTasks === 1 ? 'tarea urgente' : 'tareas urgentes'} esperando. ¿Empezamos por ahí?` });
  } else if (s.totalTasks > 0) {
    msgs.push({ tag: '✅ Agenda', text: '¡Agenda al día! Completaste todas tus tareas. Buen trabajo, {name}.' });
  }
  if (s.unassignedActivities > 0) {
    msgs.push({ tag: '🙋 Actividades', text: s.unassignedActivities === 1 ? 'Queda 1 actividad institucional sin responsable. ¿Te animas a tomarla?' : `Aún hay ${s.unassignedActivities} actividades sin responsable asignado. Elige una en la pestaña Activities.` });
  }
  if (s.myActivities > 0) {
    msgs.push({ tag: '📌 Tus actividades', text: `Tienes ${s.myActivities} ${s.myActivities === 1 ? 'actividad asignada' : 'actividades asignadas'}. Revisa sus fechas de entrega.` });
  }
  if (s.resources === 0) {
    msgs.push({ tag: '🔗 Recursos', text: 'Aún no guardas ningún enlace. Agrega los que uses siempre y los tendrás a un clic.' });
  }
  return msgs;
};
...
export const LumiCard = ({ userData, stats = {}, notifications = [], onNotificationsRead }) => {
  const { store, dispatch } = useGlobalReducer(); // <--- Conexión con el store global
  ...
  // Usamos el estado del store global para config y nombre de Lumi
  const cfg = store.lumiConfig || DEFAULT_CFG;
  const lumiName = store.lumiName || 'Lumi';

  const [draftCfg, setDraftCfg] = useState(cfg);
  const [draftName, setDraftName] = useState(lumiName);
  ...
  // Sincronizar borradores cuando el store global cambie
  useEffect(() => {
    setDraftCfg(cfg);
    ...
    try {
      ...
      if (cached) {
        ...
        dispatch({
          ...
          payload: { config: parsed, name: parsed.name || 'Lumi' }
        });
        setLoaded(true);
      }
    } catch {}
  ...
  /* ---------- Guardar personalización usando la nueva ruta Flask ---------- */
  const saveConfig = async () => {
    ...
    try {
      ...
      // Actualizamos el estado global mediante el reducer
      dispatch({
        type: 'set_lumi_config',
