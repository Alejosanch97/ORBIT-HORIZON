...
// Base del backend propio. Como el blueprint se registra con url_prefix='/api',
// aquí incluimos el /api. VITE_BACKEND_URL NO debe terminar en /api.
...
export const PlanningCLIL = ({ userData }) => {
    ...
    /* ---------- Datos curriculares (viven en el store global) ----------
       Se leen del store para no re-fetchear al cambiar de pestaña.
       Mantenemos los mismos nombres de variable para no tocar el resto del código. */
    const curriculumMaps = store.curriculumMaps || [];
    const syllabusTemplates = store.syllabusTemplates || [];
    const primeMathMaps = store.primeMathMaps || [];
    const neuroData = store.neuroData || [];
    const [loadingCurriculum, setLoadingCurriculum] = useState(false);
    const [neuroLoaded, setNeuroLoaded] = useState(false);
    // Wrappers para que las viejas llamadas setX(...) escriban en el store
    const setCurriculumMaps = (data) => dispatch({ type: 'set_curriculum_maps', payload: data });
    const setSyllabusTemplates = (data) => dispatch({ type: 'set_syllabus_templates', payload: data });
    const setPrimeMathMaps = (data) => dispatch({ type: 'set_prime_math_maps', payload: data });
    const setNeuroData = (data) => dispatch({ type: 'set_neuro_data', payload: data });
    ...
    useEffect(() => {
        // Diferimos las cargas al siguiente tick para no despachar al store durante el render
        const t = setTimeout(() => {
            fetchData();
            fetchCurriculum();
            fetchPlanReviews();
    ...
    const fetchPlanReviews = async () => {
        try {
            const resp = await fetch(`${API}/class-observations`);
   ...
   coincida con el de Profile (misma fuente de datos). */
    ...
    const fetchNeuroData = async () => {
        ...
        // Si el store ya tiene datos, no re-fetcheamos
        if ((store.neuroData || []).length > 0) { setNeuroLoaded(true); return; }
        try {
            const controller = new AbortController();
            const timeoutId = setTimeout(() => controller.abort(), 15000);
            const resp = await fetch(`${API}/neuro-stimulation`, { signal: controller.signal });
    ...
    const fetchCurriculum = async () => {
        ...
        // Si el store ya tiene el currículo cargado, reutilízalo sin ir al backend
        if ((store.curriculumMaps || []).length && (store.syllabusTemplates || []).length) {
            return { maps: store.curriculumMaps, syll: store.syllabusTemplates, prime: store.primeMathMaps || [] };
        }
        const run = (async () => {
            ...
            try {
                ...
                const [mapsResp, syllResp, primeResp] = await Promise.all([
                    fetch(`${API}/curriculum-maps`).then(r => r.json()).catch(() => []),
                    fetch(`${API}/syllabus-templates`).then(r => r.json()).catch(() => []),
                    fetch(`${API}/prime-math`).then(r => r.json()).catch(() => []),
                ]);
...
Teacher goal: ${pv.goal}`;
    ...
    const savePlanReview = async () => {
        ...
        try {
            await fetch(`${API}/class-observations`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({
    ...
    const fetchData = async (termOverride) => {
        const termToFetch = termOverride || getCurrentTerm();
        ...
        try {
            ...
            const resp = await fetch(
                `${API}/lesson-planners?term=${encodeURIComponent(termToFetch)}`,
            ...
            if (Array.isArray(data)) {
                ...
                // Espejamos al store DESPUÉS del setState (no dentro del updater)
                if (merged) dispatch({ type: 'set_plannings', payload: merged });
            } else {
