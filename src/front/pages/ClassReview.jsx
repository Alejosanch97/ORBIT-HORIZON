...
import useGlobalReducer from '../hooks/useGlobalReducer'; // O la ruta desde donde manejes tu hook global del store
...
export const ClassReview = ({ teacherList = [] }) => {
    const { store, dispatch } = useGlobalReducer();
    const reviews = store.classObservations || [];
    ...
    // Cargar desde el backend de Flask y actualizar el store
    const fetchReviewsFromBackend = async () => {
        setIsSyncing(true);
        try {
            const resp = await fetch(`${process.env.BACKEND_URL || "http://localhost:3001"}/api/class-observations`);
            if (resp.ok) {
                const data = await resp.json();
                if (Array.isArray(data)) {
                    const filtered = data.filter(r => !String(r.ID_Lesson_Ref || '').startsWith('PLAN-'));
                    dispatch({ type: 'set_class_observations', payload: filtered });
                }
            }
        } catch (e) { 
            console.error("Error fetching reviews from Flask:", e); 
        }
        setIsSyncing(false);
    };
    ...
    const handleSaveLocal = (e) => {
        ...
        // Actualizamos el store global mediante el dispatch
        dispatch({ type: 'upsert_class_observation', payload: newReview });
    ...
    const syncWithBackendAndExcel = async () => {
        setIsSyncing(true);
        try {
            for (const item of syncQueue) {
                ...
                const payload = {
                    ...
                    idValue: dataToSend.ID_Lesson_Ref,
                    data: payloadData
                };

                // 1. Guardar en tu Backend Flask
                const flaskResp = await fetch(`${process.env.BACKEND_URL || "http://localhost:3001"}/api/class-observations`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify(payload)
                });

                if (!flaskResp.ok) throw new Error("Error saving to Flask backend");

                // 2. Sincronización con Google Apps Script original
                const googlePayload = {
                    action: syncAction || 'create',
                    sheet: "Class_Observations",
                    data: payloadData,
                    ...(syncAction === 'update' ? { idField: "ID_Lesson_Ref", idValue: dataToSend.ID_Lesson_Ref } : {})
                };
                await fetch(API_URL, { method: 'POST', body: JSON.stringify(googlePayload) });
            }

            setSyncQueue([]);
            fetchReviewsFromBackend();
            alert("🚀 Success! Synced with Backend and Google Sheets.");
        } catch (e) { 
            console.error(e);
        ...
        <div className="planning-wrapper">
            <header className="page-header">
                <div className="title-section">
                    ...
                    <p>Monitoring and feedback sync via Store.</p>
                </div>
                <div className="header-actions">
                    {isSyncing && <div className="sync-loader">⏳ Syncing...</div>}
                    {syncQueue.length > 0 && (
                        <button className="btn-sync" onClick={syncWithBackendAndExcel} disabled={isSyncing}>
                            🔄 Push {syncQueue.length} to Server
                        </button>
                    )}
                ...
                <table className="modern-table">
                    ...
                    <tbody>
                        {filteredReviews.length > 0 ? filteredReviews.slice(0, 20).map((rev, i) => (
                            <tr key={rev.ID_Lesson_Ref || i} className={rev.isLocal ? "row-local" : ""}>
